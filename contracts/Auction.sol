...
contract Auction {
    ...
    function getBidWinner(uint256 _tokenId, address _nftContract) public view returns(address payable,uint256){
        AuctionDetails memory auction= tokenIdToAuction[_nftContract][_tokenId];
       require(auction.highestBid >= 0 ,"No bids received");
        return (payable(auction.highestBidder),auction.highestBid);
    }
